...
create_arp_response(ip_addr_t *adr)
{
  ...
  struct pbuf *p = pbuf_alloc(PBUF_RAW, sizeof(struct eth_hdr) + sizeof(struct etharp_hdr), PBUF_RAM, NULL);
  if(p == NULL) {
    FAIL_RET();
  }
...
START_TEST(test_etharp_table)
{
  ...
  if (pcb != NULL) {
    ...
    for(i = 0; i < ARP_TABLE_SIZE; i++) {
      struct pbuf *p = pbuf_alloc(PBUF_TRANSPORT, 10, PBUF_RAM, NULL);
      fail_unless(p != NULL);
      if (p != NULL) {
