...
void ibv_attach_device (struct netif *netif)
{
  ...
  fr.tcp_spec_info.val.dst_port = bswap_16(80);
...
low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  ibvif->sent_count++;
  if ((p->payload >= ibvif->send_buf) && (p->payload <= (ibvif->send_buf+ibvif->buf_size))) {
    if (infini_post_send(ibvif, p->payload, p->tot_len)) {
      perror("tapif: write 1");
    }
  } else {
    perror("tapif: invalid write");
  }
...
ibvif_init(struct netif *netif)
{
  ...
  ibvif[ibvif_temp->thread->cpu] = ibvif_temp; 
